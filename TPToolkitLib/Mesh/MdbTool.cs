...
namespace TPToolkitLib.Mesh
{
    public class MdbTool
    {
        ...
        public void XMdbTo1Obj(string[] mdbs, string objPath, string textureDirectory, bool exportLods, IProgress<int> progress, IProgress<string> logs)
        {
            try
            {
                ...
                using (StreamWriter objWriter = new StreamWriter(File.Open(objPath, FileMode.Create, FileAccess.ReadWrite)))
                {
                    ...
                    using (StreamWriter mtlWriter = new StreamWriter(File.Open(mtlPath, FileMode.Create, FileAccess.ReadWrite)))
                    {
                        ...
                        try
                        {
                            ...
                            watch.Restart();
                            WriteMtl(mtlWriter, textureDirectory);
                            watch.Stop();
                            logs.Report("Done in " + TimeSpanFormat.Get(watch.Elapsed) + "\n");
                        }
                        catch (Exception ex)
        ...
        /// <param name="progress">Progress on the progress bar.</param>
        /// <param name="logs">Logs in the text box.</param>
        public void XMdbToXObj(string[] mdbs, string objFolderPath, string textureDirectory, bool exportLods, IProgress<int> progress, IProgress<string> logs)
        {
            try
            {
                var watch = new System.Diagnostics.Stopwatch();
        ...
        /// <exception cref="Exception"></exception>
        private void WriteMtl(StreamWriter writer, string textureDirectory)
        {
            try
            {
                foreach (var mat in finalMat)
                {
                    ...
                    writer.WriteLine("map_Kd " + textureDirectory + Path.ChangeExtension(mat.TexName, "dds"));
                    writer.WriteLine();
                }
            }
            catch
