...
namespace TPToolkitLib.Mesh
{
    public class GlbTool
    {
        ...
        public void XMdbTo1Glb(string[] mdbs, string glbPath, string textureDirectory, bool exportLods, IProgress<int> progress, IProgress<string> logs)
        {
            try
            {
                ...
                watch.Restart();
                WriteGlb(glbPath, textureDirectory, logs);
        ...
        /// <param name="progress">Progress on the progress bar.</param>
        /// <param name="logs">Logs in the text box.</param>
        public void XMdbToXGlb(string[] mdbs, string glbFolderPath, string textureDirectory, bool exportLods, IProgress<int> progress, IProgress<string> logs)
        {
            try
            {
                var watch = new System.Diagnostics.Stopwatch();
        ...
        private void WriteGlb(string glbPath, string textureDirectory, IProgress<string> logs)
        {
            ...
            for (int i = 0; i < finalMdbMaterials.Count; i++)
            {
                ...
                try
                {
                    var pngBytes = ConvertDdsToPngBytes(textureDirectory + Path.ChangeExtension(finalMdbMaterial.TextureName, "dds"));
